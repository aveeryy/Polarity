...
class MPEGDASHStream(StreamProtocol):
    ...
    def open_playlist(self):
        ...
        for adap_set in self.manifest_data['MPD']['Period']['AdaptationSet']:
            if adap_set['@contentType'] == 'video':
                vprint(lang['penguin']['protocols']['picking_best_stream_0'],
                       3, 'penguin/dash', 'debug')
                for repr in adap_set['Representation']:
                    resolution_list.append((repr, int(repr['@height'])))
                resolution = min(
                    resolution_list,
                    key=lambda x: abs(x[1] - self.options['resolution']))
                ...
                if len(streams) > 1:
                    vprint(
                        ...
                        3, 'penguin/dash', 'debug')
                    bandwidth_values = [
                        int(s[0]['@bandwidth']) for s in streams
                    ]
                    stream = streams[bandwidth_values.index(
                        max(bandwidth_values))][0]
                else:
                    stream = streams[0][0]
                vprint(
                    lang['penguin']['protocols']['selected_stream'] %
                    stream['BaseURL'], 3, 'penguin/dash', 'debug')
                self.get_stream_fragments(stream, 'video', 'video')
            elif adap_set['@contentType'] == 'audio':
                vprint(lang['penguin']['protocols']['picking_best_stream_2'],
                       3, 'penguin/dash', 'debug')
        ...
        def build_segment_pool(media_type=str):
            ...
            segments = [
                ...
                Segment(
                    url=self.stream_url,
                    number=list(
                        representation['SegmentList']['SegmentURL']).index(s),
            ...
            seg_pool = SegmentPool(
                segments, media_type,
                f'{media_type}{self.processed_tracks[media_type]}', track_id,
                DASHPool)
        ...
        def create_init_segment(pool: str) -> None:
            self.segment_pool.segments.append(
                Segment(url=self.stream_url,
                        ...
                        ext=get_extension(self.stream_url),
                        duration=None,
                        byte_range=representation['SegmentList']
                        ['Initialization']['@range']))

        self.stream_url = urljoin(self.url, representation['BaseURL'])
        vprint(lang['penguin']['protocols']['getting_stream'], 3,
               'penguin/dash', 'debug')
    ...
    def extract(self):
        self.retries = Retry(total=30,
                             backoff_factor=1,
                             status_forcelist=[502, 503, 504, 403, 404])
        ...
        vprint(lang['penguin']['protocols']['getting_playlist'],
               3,
               module_name='penguin/dash',
               error_level='debug')
        self.open_playlist()
        return {
            'segment_pools': self.segment_pools,
            'tracks': self.processed_tracks
        }
