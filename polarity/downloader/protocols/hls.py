...
class HTTPLiveStream(StreamProtocol):
    ...
    def open_playlist(self):
        ...
        if self.parsed_data['is_variant']:
            ...
            self.resolutions = [
                (s,
                 int(s['stream_info']['resolution'].
                     split('x')[1] if 'resolution' in s['stream_info'] else 0))
            ...
            self.streams = [
                s for s in self.resolutions if s[1] == self.resolution[1]
            ]
            ...
            if len(self.streams) > 1:
                ...
                self._stream = self.streams[self.bandwidth_values.index(
                    max(self.bandwidth_values))][0]
            else:
    ...
    def get_stream_fragments(self,
        ...
        def build_segment_pool(media_type=str):
            ...
            segments = [
                ...
                Segment(
                    ...
                    key={
                        'video':
                        ContentKey(s['key']['uri'], None, s['key']['method'])
                        if 'key' in s else None,
                        'audio':
                        ContentKey(s['key']['uri'], None, s['key']['method'])
                        if 'key' in s else None,
                    },
        ...
        def create_init_segment(pool: str) -> None:
            self.segment_pool.segments.append(
                Segment(url=urljoin(self.stream_url,
    ...
    def extract(self):
        ...
        self.open_playlist()
        self.get_stream_fragments(self._stream,
                                  only_subtitles=self.stream.extra_sub)
        return {
